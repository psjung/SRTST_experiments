...
class  StatsAccumulator  implements TreeWalkerStatsAccumulator {
  ...
  void copyToStats(  DatabaseStats stats){
   ...
   private void  hook363__wrappee__Statistics  (  IN node){
  }

	
  protected void hook363(  IN node){
    verifyNode(node);
    hook363__wrappee__Statistics(node);
  }

	
   private void  hook364__wrappee__Statistics  (  BIN node){
  }

	
  protected void hook364(  BIN node){
    verifyNode(node);
    hook364__wrappee__Statistics(node);
  }

	
   private void  hook365__wrappee__Statistics  (  DIN node){
  }

	
  protected void hook365(  DIN node){
    verifyNode(node);
    hook365__wrappee__Statistics(node);
  }

	
   private void  hook366__wrappee__Statistics  (  DBIN node){
  }

	
  protected void hook366(  DBIN node){
    verifyNode(node);
    hook366__wrappee__Statistics(node);
  }

	
   private void  hook367__wrappee__Statistics  (  DupCountLN node){
  }

	
  protected void hook367(  DupCountLN node){
    verifyNode(node);
    hook367__wrappee__Statistics(node);
  }
