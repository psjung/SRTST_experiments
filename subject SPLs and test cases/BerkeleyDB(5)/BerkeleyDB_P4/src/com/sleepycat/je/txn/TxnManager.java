 ...
 */
   private void  registerTxn__wrappee__base  (  Txn txn) throws DatabaseException {
    allTxns.add(txn);
 ...
 */
  void registerTxn(  Txn txn) throws DatabaseException {
    allTxnLatch.acquire();
    registerTxn__wrappee__base(txn);
    allTxnLatch.release();
  }
 ...
 */
   private void  unRegisterTxn__wrappee__base  (  Txn txn,  boolean isCommit) throws DatabaseException {
    allTxns.remove(txn);
    this.hook828(txn);
 ...
 */
  void unRegisterTxn(  Txn txn,  boolean isCommit) throws DatabaseException {
    allTxnLatch.acquire();
    try {
      unRegisterTxn__wrappee__base(txn,isCommit);
    }
  finally {
      allTxnLatch.release();
    }
 ...
 */
  public long getFirstActiveLsn() throws DatabaseException {
   ...
   private void  hook821__wrappee__base  (  EnvironmentImpl env) throws DatabaseException {
  }

	
  protected void hook821(  EnvironmentImpl env) throws DatabaseException {
    allTxnLatch=LatchSupport.makeLatch(DEBUG_NAME,env);
    hook821__wrappee__base(env);
  }

	
   private void  hook822__wrappee__base  (  EnvironmentImpl env) throws DatabaseException {
    if (env.isNoLocking()) {
  ...
  protected void hook822(  EnvironmentImpl env) throws DatabaseException {
    if (EnvironmentImpl.getFairLatches()) {
      ...
      hook822__wrappee__base(env);
    }
  }

	
   private long  hook823__wrappee__base  (  long firstActive) throws DatabaseException {
  ...
  protected long hook823(  long firstActive) throws DatabaseException {
    allTxnLatch.acquire();
    try {
      firstActive=hook823__wrappee__base(firstActive);
    }
  finally {
      allTxnLatch.release();
    }
  ...
  protected void hook827(  boolean isCommit) throws DatabaseException {
   ...
   private void  hook828__wrappee__base  (  Txn txn) throws DatabaseException {
  }
  ...
  protected void hook828(  Txn txn) throws DatabaseException {
    ...
    hook828__wrappee__base(txn);
  }

	
   private void  hook829__wrappee__base  () throws DatabaseException {
  }
  ...
  protected void hook829() throws DatabaseException {
    ...
    hook829__wrappee__base();
  }

	
   private void  hook830__wrappee__base  () throws DatabaseException {
  }
 ...
 */
  public LockStats lockStat(  StatsConfig config) throws DatabaseException {
   ...
   private void  hook820__wrappee__Statistics  (  StatsConfig config,  TransactionStats stats) throws DatabaseException {
  ...
  protected void hook820(  StatsConfig config,  TransactionStats stats) throws DatabaseException {
    allTxnLatch.acquire();
    try {
      hook820__wrappee__Statistics(config,stats);
    }
  finally {
      allTxnLatch.release();
    }
